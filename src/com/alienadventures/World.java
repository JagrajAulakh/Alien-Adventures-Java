...
public class World {
	...
	private void loadWorld() {
		player = new Player(this, Player.PLAYER_BROWN);
		for (int i = 0; i < 10; i++) {
			Particle p = new Particle(this, player.getX() + player.getWidth() / 2, player.getY() + player.getHeight() / 2, player.getType());
			objects.add(p);
		}
		objects.add(new Platform(this, Game.WIDTH / 2.0 - 100, 500, 1000, 20));
		objects.add(new Platform(this, Game.WIDTH / 2.0 - 300, 590, 200, 20));
		objects.add(new Box(this, Game.WIDTH / 2.0-25, 0));
		objects.add(new Box(this, Game.WIDTH / 2.0 + 25, 0));
