...
public class Window extends ScreenObject implements ObjectListener {
	...
	public Window(int x, int y, String title) {
		...
		scale = 0.01;
		winSurf = new BufferedImage(Game.WIDTH, Game.HEIGHT, BufferedImage.TYPE_INT_ARGB_PRE);
	...
	public void render(Graphics g, Camera camera) {
		...
		BufferedImage windowImage = Resources.buttonImages.get(24);
		// Draws main window image
		gWin.drawImage(windowImage, (int)windowCamera.getOffsetX(), (int)windowCamera.getOffsetY(), null);
		// Draws window title
		...
		if (1 <= animationCounter && animationCounter < animationCounterMax) {
			Resources.drawCentered(g, Resources.scale(winSurf, scale), (int)(windowCamera.getOffsetX() + windowImage.getWidth() / 2), (int)(windowCamera.getOffsetY() + windowImage.getHeight() / 2));
		} else {
			g.drawImage(winSurf, 0, 0, null);
		}
